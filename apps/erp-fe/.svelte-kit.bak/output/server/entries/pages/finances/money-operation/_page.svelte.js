...
const MoneyOperationTable = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let { data = void 0 } = $$props;
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  return `<div class="overflow-x-auto text-primary-content mx-auto"> <table class="table"> <thead class="text-primary-content" data-svelte-h="svelte-1ppnexn"><tr><th>ID</th> <th>Operation Type</th> <th>Operation Category</th> <th>Operation Description</th> <th>Amount</th> <th>Currency</th> <th>Effective Date</th></tr></thead> <tbody>${data !== void 0 ? `${each(data.content, (moneyOperation) => {
    return `<tr><td>${escape(moneyOperation.uuid)}</td> <td>${escape(moneyOperation.operationCategory.operationType)}</td> <td>${escape(moneyOperation.operationCategory.operationName)}</td> <td>${escape(moneyOperation.operationDescription)}</td> <td>${escape(moneyOperation.amount.value)}</td> <td>${escape(moneyOperation.amount.currencyCode)}</td> <td>${escape(moneyOperation.effectiveDate)}</td> </tr>`;
  })}` : ``}</tbody></table></div>`;
});
