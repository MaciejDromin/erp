...
const securedExternalApiRequest = async (
  ...
  if (authHeader === undefined) {
    ...
    return new Response(null, {status: 204, headers: {location: "/login", redirected: true}})
  }
  const ret = await fetch(url, {
    method: method,
    body: JSON.stringify(body),
...
const tryRefreshingToken = async (cookies) => {
  ...
  if (refreshToken === undefined) return new Response(null, {status: 204, headers: {location: "/login", redirected: true}})
  ...
  if (refreshResp.status === 401) return new Response(null, {status: 204, headers: {location: "/login", redirected: true}})
