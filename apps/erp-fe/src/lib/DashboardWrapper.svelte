<script>
  ...
  const changeDashboard = async () => {
    const dashboardRet = await apiRequest("/dashboards/" + selectedDashboard, HttpMethods.GET)
    dashboardData = await dashboardRet.json()
  }
...
<div class="flex flex-col w-full h-full">
  <div class="flex flex-row w-full h-full justify-between py-8">
    <select class="select w-full max-w-xs text-white"
      bind:value={selectedDashboard} on:change={() => changeDashboard()}>
      {#each selectionOptions as opt}
        <option id={opt.id} selected={opt.defaultForType} value={opt.id}>{opt.name}</option> 
      {/each}
    </select>
    <div id="options">
