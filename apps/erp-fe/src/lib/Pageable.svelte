...
<div class="flex flex-col">
    ...
    <div class="join flex mx-auto">
        <button class={`join-item btn disabled:opacity-75 ${!doesLowerPageExists(currentPage) ? "btn-disable opacity-75" : ""}`} on:click|preventDefault={() => fetchData(--currentPage, additionalSearch)}>«</button>
        <button class="join-item btn btn-disable">Page {currentPage + 1}</button>
        <button class={`join-item btn disabled:opacity-75 ${!doesHigherPageExists(currentPage, maxPage) ? "btn-disable opacity-75" : ""}`} on:click|preventDefault={() => fetchData(++currentPage, additionalSearch)}>»</button>
    </div>
</div>
<style>
.btn-disable {
