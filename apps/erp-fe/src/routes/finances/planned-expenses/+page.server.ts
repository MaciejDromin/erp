...
export const actions = {
    create: async ({ request }) => {
        ...
        await unsecuredExternalApiRequest(FINANCES_URL +
            '/finances/planned-expenses', HttpMethods.POST, body)
    },
    abandon: async ({ request }) => {
        const data = await request.formData();
        await unsecuredExternalApiRequest(FINANCES_URL +
            `/finances/planned-expenses/${data.get("plannedExpensesId")}/abandon`, HttpMethods.PATCH)
    },
    complete: async ({ request }) => {
        ...
        await unsecuredExternalApiRequest(FINANCES_URL +
            `/finances/planned-expenses/${data.get("plannedExpensesId")}/complete`, HttpMethods.PATCH, body)
    }
} satisfies Actions;
