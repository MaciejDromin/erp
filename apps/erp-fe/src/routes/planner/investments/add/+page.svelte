<script lang="ts">
    ...
    let name = ''
    let currency = ''
    let phases:{name:string, phaseTime:number|undefined, currency:string}[] = []
    let materialsPerPhase:[{materialId:string, quantity:number|undefined}][] = []
    ...
    selectedMaterials.subscribe((mat) => {
        ...
        materialsPerPhase[mat.phasePos][mat.materialPos].materialId = mat.selectedMaterialId
        materialsPerPhase = materialsPerPhase
    })
    ...
    <form method="POST" class="mx-auto flex flex-col gap-3 py-6 min-w-[40rem]">
        <div class="flex flex-row gap-3 mx-auto">
            ...
            <input
                ...
                bind:value={currency}
            />
            <input name="creation" class="hidden" value={JSON.stringify({
                name: name,
                currency: currency,
                investmentPhases: createPhases(phases, materialsPerPhase, labourPerPhase)
            })}/>
        </div>
        <div class="flex flex-col gap-3">
            {#each phases as phase, i}
                <div class="flex flex-col w-full">
                    <h1 class="text-xl mx-auto text-black py-3">Phase {i + 1}</h1>
                    ...
                    {#each materialsPerPhase[i] as material, mi}
                        <h1 class="text-xl mx-auto text-black py-3">Material {mi + 1}</h1>
                        <div class="h-[2px] w-full bg-black rounded-full my-3"></div>
                        <div class="flex flex-row gap-3 mx-auto">
                            <div class="mr-auto">
                                <Modal
                                    modalId={`material-modal-ph-${i}-m-${mi}`}
                                    buttonName={determineMaterialButtonName(materialsPerPhase[i][mi].materialId)}
                                >
                                    <Pageable
                                        endpoint="/planner/materials"
                                        component={MaterialsTable}
                                        additionalParams={{phasePos: i, materialPos: mi}}
                                    />
                                </Modal>
                            </div>
                            <input
                    ...
                    {/each}
                    <button on:click|preventDefault={() => addMaterial(i)} class="btn btn-primary mx-auto my-3">Add Material</button>
                    {#each labourPerPhase[i] as labour, li}
                        <h1 class="text-xl mx-auto text-black py-3">Labour {li + 1}</h1>
                        <div class="h-[2px] w-full bg-black rounded-full my-3"></div>
                        <div class="flex flex-row gap-3 mx-auto">
                            <div class="mr-auto">
                                <Modal
                                    modalId={`labour-modal-ph-${i}-m-${li}`}
                                    buttonName={determineLabourButtonName(labourPerPhase[i][li].labourId)}
                                >
                                    <Pageable
                                        endpoint="/planner/labours"
                                        component={LaboursTable}
                                        additionalParams={{phasePos: i, labourPos: li}}
                                    />
                                </Modal>
                            </div>
                            <input
                    ...
                    {/each}
                    <button on:click|preventDefault={() => addLabour(i)} class="btn btn-primary mx-auto my-3">Add Labour</button>
                </div>
            {/each}
        </div>
        <button on:click|preventDefault={addPhase} class="btn btn-primary mx-auto">Add Phase</button>
